    ...
    protected NormalizedResult normalizeCollection(Collection<String> collection) {
        ...
        for (String currStartingValue : collection) {
            ...
            if (normalizedString == null) {
                Integer tempBlankCount = blankToCount.get(currStartingValue);
                if (tempBlankCount == null) {
                    tempBlankCount = 0;
                }
                tempBlankCount++;
                blankToCount.put(currStartingValue, tempBlankCount);
            } else {  // else, non-blank so normalize

                List<String> tempMergedValues = normalizedValueToOrigValues.get(normalizedString);
                if (tempMergedValues == null) {
                    tempMergedValues = new ArrayList<>();
                }
                tempMergedValues.add(currStartingValue);
                normalizedValueToOrigValues.put(normalizedString, tempMergedValues);
            }
        }
